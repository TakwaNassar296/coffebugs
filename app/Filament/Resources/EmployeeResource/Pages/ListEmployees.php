    ...
    public function getTabs(): array
    {
        ...
        return [
            'all' => Tab::make(__('admin.all_employees'))
                ...
                ->badgeColor('primary')
                ->modifyQueryUsing(fn (Builder $query) => $query->role('employee')),
            
            'total_points' => Tab::make(__('admin.total_points'))
                ->modifyQueryUsing(function (Builder $query) use ($auth) {
                    return $query->role('employee')
                        ->when(!$auth->hasRole('super_admin'), fn($q) => $q->where('branch_id', $auth->branch_id))
                        ->orderBy('total_points', 'desc');
                })
                ->badge(number_format($baseQuery->sum('total_points') ?? 0, 2))
                ->badgeColor('success'),

...
class ExternalsMaterailController extends Controller
{
    use ApiResponse;

    /**
     * Get external materials for the authenticated branch
     * Returns only materials where material_type = 'external'
     */
    public function materials()
    {
        ...
        // Get branch materials where the related material has material_type = 'external'
        $materials = BranchMaterial::where('branch_id', $branchId)
            ->whereHas('material', function ($query) {
                $query->where('material_type', 'external');
            })
            ->with('material.category')
            ->get();

        return $this->successResponse(
            'External materials retrieved successfully.',
    ...
    public function createMaterialRequest(Request $request)
    {
        ...
        // Validate that material is external type
        if (!$material || $material->material_type !== 'external') {
            return $this->errorResponse(
                'Only external materials can be requested through this endpoint.',
                422
            );
        }
        ...
        try {
            ...
            return $this->successResponse(
                ...
                [
                    ...
                    'type' => $materialRequest->material->material_type ?? 'external',
    ...
    public function getMaterialRequests(Request $request)
    {
        ...
        // Only get external material requests
        ...
        $data = $requests->map(function ($request) {
            return [
                ...
                'type' => $request->material?->material_type ?? 'external',
